...
export default function EditBlog({ params }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const authorResponse = await insertData('api/author/getall', authorPerameter , true);
                console.log(authorResponse, ' >>> Author Data');
                if (authorResponse.status) {
                    setAuthorData(authorResponse.data.list);
                }
                const response = await insertData("api/blog/get-blog-id", requestData, true);

                if (response.status) {
                    setBlogData(response.data);
                    const bannerImageArray = [];
                    bannerImageArray.push(response.data.image);
                    setFilePreviews(bannerImageArray);
                    setErrorMessage('');
                } else {
    ...
    const handleSubmit = async (values, { resetForm, setErrors }) => {
        ...
        try {
            ...
            const response = await insertData(`api/blog/edit/${blogData.id}`, { title_en, title_fr, description_en, description_fr, author_id, image }, true);
                    ...
                    <LayoutAdmin>
                        ...
                        <Formik
                            ...
                            initialValues={{
                                ...
                                description_fr: blogData?.description_fr || '',
                                picture_img: blogData?.image || '',
                            ...
                            {({ errors, touched, handleChange, handleBlur, setFieldValue, values }) => (
                                <Form>
                                    <div>
                                        ...
                                        <button type="submit" className="tf-btn primary" onClick={() => setShowErrorPopup(!showErrorPopup)}>Update Blog</button>
                                    </div>
                                    {/* Error Popup */}
