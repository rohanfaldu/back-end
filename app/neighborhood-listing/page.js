...
export default function ProjectAmenitiesListing() {
  const [neighborhoods, setNeighborhoods] = useState([]); // Store properties for the current page
  ...
  const fetchNeighborhoods = async (page = variablesList.currentPage, term = '', status = '') => {
    setLoading(true);
    try {
      const requestData = {
        page,
      ...
      const response = await insertData("api/neighborhood/", requestData, true);
      if (response.status) {
        const { neighborhoods, totalCount, totalPages, currentPage } = response.data;
  ...
  useEffect(() => {
    fetchNeighborhoods(pagination.currentPage, searchTerm, statusFilter);
  ...
  const handleDelete = async (id) => {
    console.log(id);
    try {
      const response = await deletedData(`api/neighborhood/delete/${id}`);
          ...
          <LayoutAdmin>
            <div className="wrap-dashboard-content">
              ...
              <div class="top d-flex justify-content-between align-items-center">
                  <h6 className="title">Neighborhood Listing</h6>
                  <Link className="remove-file tf-btn primary" href="/create-neighborhood">Create Neighborhood</Link>
                </div>
                          ...
                          <tbody>
                            {neighborhoods.map((property) => (
                             ...
                             <td>
                               ...
                               <li className="edit">
                                        <Link href={`/edit-neighborhood/${property.id}`} className="item">
                                          <Image 
